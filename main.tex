...
\importname{Autocorrelation}{xcorr(u)}

\importname{Crosscorrelation}{xcorr(u,v)}
...
\important{$e(k)\in\mathcal{N}(0,\lambda)\Rightarrow\begin{cases}\expe{e(k)}=0\text{ (zero mean)}\\\expe{|e(k)|^2}=\lambda\text{ (varianve)}\end{cases}$}
...
\importname{Spectral estimation}{$\frac{\hat{\phi}_{yu}\ejon}{\hat{\phi}_u\ejon}$}
...
\subsubsection{Spectral estimation for periodic signals}
...
\item Autocorrelation zero except for $\tau=0$.
\end{itemize}
...
\important{$\tilde{\phi}_v\ejon\approx\frac{1}{N}\frac{1}{2\pi}\int_{-\pi}^\pi W_\gamma\ejzw\left|Y_N\ejo-\tilde{G}\ejo U_N\ejo\right|^2d\zeta\approx \tilde{\phi}\ejon-\frac{\left|\tilde{\phi}_{yu}\ejon\right|^2}{\tilde{\phi}_u\ejon}$}

How much energy is accounted for by the model? How much by noise?

\mportant{$\phi_v = \phi_y(1-\frac{|\phi_{yu}|}{\phi_y\phi_u}$}
